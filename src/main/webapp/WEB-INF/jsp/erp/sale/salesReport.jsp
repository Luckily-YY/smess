...
<body class="no-skin">
			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="sale/insaleReport.do" method="post" name="Form" id="Form">
								...
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											...
											<td class='center'><b><font style="color: red">${var.SALECOUNT}</font></b></td>
											<td class='center'><b><font style="color: red">${var.SALEPRICE}</font></b>&nbsp;元</td>
										</tr>
	...
	<script type="text/javascript">
		...
		//排序
		var zj = 'order by SALEPRICE';
		var xl = 'order by SALECOUNT';
		function orderby(msg){
			var value= "${pd.forderby }";
			if('zj' == msg){
				if(value== zj){
					zj = 'order by SALEPRICE desc';
				}else{
					zj = 'order by SALEPRICE';
				}
				$("#forderby").val(zj);
			}
			if('xl' == msg){
				if(value== xl){
					xl = 'order by SALECOUNT desc';
				}else{
					xl = 'order by SALECOUNT';
				}
				$("#forderby").val(xl);
			}
			tosearch();
		...
		//导出excel
		function toExcel(){
			$("#Form").attr("action", "<%=basePath%>sale/exportExcel.do");
			setTimeout(function() {
				$("#Form").attr("action", "<%=basePath%>sale/insalesReport.do");
	        }, 500 );
			$("#Form").submit();
		}
	</script>
