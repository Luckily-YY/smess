            ...
            try {
                ...
                if (!"sqlserver".equals(dbtype)) {
                    ...
                    if ("yes".equals(remoteDB)) {//数据库另外一台服务器上(和tomcat不在同一台服务器上)
                        ...
                        bb.writeUTF("" + commandStr + "" + sqlpath);    //发送指令给服务端
                        bb.flush();
                        Boolean llm = true;
                        while (llm) {
                        ...
                        FileUtil.createDir(sqlpath + "/backups.fh");
                        ...
                        File file = new File(f,("".equals(tableName) ? databaseName + "_" + ffilename : tableName + "_" + ffilename) + ".sql");
                        if(!file.exists()) {
                            file.createNewFile();
                        }
                       ...
                       /* InputStream in = p.getInputStream();// 控制台的输出信息作为输入流*/
                        InputStreamReader inStream = new InputStreamReader(p.getInputStream(), "utf-8");
                        // 要用来做导入用的sql目标文件：
