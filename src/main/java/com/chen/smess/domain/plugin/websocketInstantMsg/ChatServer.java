package com.chen.smess.domain.plugin.websocketInstantMsg;
...
public class ChatServer extends WebSocketServer{
	...
	public void onMessage(WebSocket conn, String message){
		...
		if(null != message && message.startsWith("SmessAdmin")){
			this.userjoin(message.replaceFirst("SmessAdmin", ""),conn);
		}if(null != message && message.startsWith("LeaveSmess")){
			this.userLeave(conn);
		}if(null != message && message.contains("SmessAdmin886")){
			String toUser = message.substring(message.indexOf("SmessAdmin886")+13, message.indexOf("SmessAdmin888"));
			message = message.substring(0, message.indexOf("SmessAdmin886")) +"[私信]  "+ message.substring(message.indexOf("SmessAdmin888")+13, message.length());
