package com.chen.smess.app.controller.system.head;

import com.chen.smess.app.controller.base.BaseController;
import com.chen.smess.domain.common.utils.*;
import com.chen.smess.domain.common.utils.mail.SimpleMailSender;
import com.chen.smess.domain.service.system.fhsms.FhsmsManager;
import com.chen.smess.domain.service.system.user.UserManager;
import com.chen.smess.domain.service.system.userphoto.UserPhotoManager;
...
public class HeadController extends BaseController {
	...
	public Object sendEmail(){
		...
		if(null != strEMAIL && !"".equals(strEMAIL)){
			String strEM[] = strEMAIL.split(",smess,");
			if(strEM.length == 4){
				if("yes".endsWith(isAll)){
					try {
						List<PageData> userList = new ArrayList<PageData>();
	...
	public ModelAndView goEditEmail() throws Exception{
		...
		if(null != strSMS1 && !"".equals(strSMS1)){
			String strS1[] = strSMS1.split(",smess,");
			if(strS1.length == 2){
		...
		if(null != strSMS2 && !"".equals(strSMS2)){
			String strS2[] = strSMS2.split(",smess,");
			if(strS2.length == 2){
		...
		if(null != strFWATERM && !"".equals(strFWATERM)){
			String strFW[] = strFWATERM.split(",smess,");
			if(strFW.length == 5){
		...
		if(null != strIWATERM && !"".equals(strIWATERM)){
			String strIW[] = strIWATERM.split(",smess,");
			if(strIW.length == 4){
	...
	public ModelAndView saveSys() throws Exception{
		...
		Tools.writeFile(Const.EMAIL,pd.getString("SMTP")+",smess,"+pd.getString("PORT")+",smess,"+pd.getString("EMAIL")+",smess,"+pd.getString("PAW"));	//写入邮件服务器配置
		Tools.writeFile(Const.SMS1,pd.getString("SMSU1")+",smess,"+pd.getString("SMSPAW1"));	//写入短信1配置
		Tools.writeFile(Const.SMS2,pd.getString("SMSU2")+",smess,"+pd.getString("SMSPAW2"));	//写入短信2配置
		mv.addObject("msg","OK");
	...
	public ModelAndView saveSys2() throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		Tools.writeFile(Const.FWATERM,pd.getString("isCheck1")+",smess,"+pd.getString("fcontent")+",smess,"+pd.getString("fontSize")+",smess,"+pd.getString("fontX")+",smess,"+pd.getString("fontY"));	//文字水印配置
		Tools.writeFile(Const.IWATERM,pd.getString("isCheck2")+",smess,"+pd.getString("imgUrl")+",smess,"+pd.getString("imgX")+",smess,"+pd.getString("imgY"));	//图片水印配置
		Watermark.fushValue();
		mv.addObject("msg","OK");
		mv.setViewName("save_result");
		return mv;
	...
	public ModelAndView saveSys3() throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		Tools.writeFile(Const.WEBSOCKET,pd.getString("WIMIP")+",smess,"+pd.getString("WIMPORT")+",smess,"+pd.getString("OLIP")+",smess,"+pd.getString("OLPORT")+",smess,"+pd.getString("FHsmsSound"));	//websocket配置
		mv.addObject("msg","OK");
		mv.setViewName("save_result");
		return mv;
	}
	...
	public ModelAndView saveSys4() throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		Tools.writeFile(Const.LOGINEDIT,pd.getString("isZhuce")+",smess,"+pd.getString("isMusic"));	//登录页面配置
		mv.addObject("msg","OK");
		mv.setViewName("save_result");
		return mv;
	}
