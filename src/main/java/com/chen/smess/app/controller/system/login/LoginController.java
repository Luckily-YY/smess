package com.chen.smess.app.controller.system.login;

import com.chen.smess.app.controller.base.BaseController;
import com.chen.smess.domain.common.utils.*;
import com.chen.smess.domain.model.system.Menu;
import com.chen.smess.domain.model.system.Role;
...
import com.chen.smess.domain.service.system.fhlog.FHlogManager;
import com.chen.smess.domain.service.system.loginimg.LogInImgManager;
import com.chen.smess.domain.service.system.menu.MenuManager;
import com.chen.smess.domain.service.system.role.RoleManager;
...
public class LoginController extends BaseController {
    ...
    public Object login() throws Exception {
        ...
        pd = this.getPageData();
        String errInfo = "";
        String KEYDATA[] = pd.getString("KEYDATA").replaceAll("smesslogin1", "").replaceAll("smesslogin2", "").split(",smess,");
        System.out.println(KEYDATA[0] + "," + KEYDATA[1] + "," + KEYDATA[2]);
    ...
    public ModelAndView defaultPage() throws Exception {
        ...
        /*前台销售金额*/
        PageData salepd = saleReportService.priceSum(pd);
        if (salepd != null) {
            outjine = salepd.getString("SALEPRICE").toString();
        }
        if (salepd != null && outjinepd != null) {
            BigDecimal sale = new BigDecimal(salepd.getString("SALEPRICE").toString());
            BigDecimal out = new BigDecimal(outjinepd.getString("ZPRICE").toString());
            outjine = sale.add(out).toString();
        }
        ...
        if (goodslist != null && goodslist.size() != 0) {
            ...
            for (PageData goodspd : goodslist) {
                BigDecimal count = new BigDecimal(goodspd.get("GCOUNT").toString());
                BigDecimal price = new BigDecimal(goodspd.get("LPRICE").toString());
                String newjine = count.multiply(price).toString();    //货架上商品的库存金额

                BigDecimal bjine = new BigDecimal(jine);
                BigDecimal bnewjine = new BigDecimal(newjine);
                jine = bjine.add(bnewjine).toString();
            }
            kucunjine = jine;
        }

        if (null != kucunjinepd && (goodslist != null && goodslist.size() != 0))
        {
            BigDecimal kucun = new BigDecimal(kucunjinepd.get("ZPRICE").toString());
            String jine = "0";
            for (PageData goodspd : goodslist) {
                BigDecimal count = new BigDecimal(goodspd.get("GCOUNT").toString());
                BigDecimal price = new BigDecimal(goodspd.get("LPRICE").toString());
            ...
            BigDecimal goodsjine = new BigDecimal(jine);
            kucunjine = goodsjine.add(kucun).toString();
        }

        DecimalFormat format = new DecimalFormat("#.00");
        pd.put("kucunjine", format.format(Double.parseDouble(kucunjine)));                //总库存金额

        pd.put("days", 30);
        pd.put("newUuserCount", customerService.listAll(pd).size());    //30天内新增客户数
        PageData outjinepd30 = outkuService.priceSum(pd);
        PageData salepd30 = saleReportService.priceSum(pd);
        String outjine30 = "0";
        if (null != outjinepd30) {
            outjine30 = outjinepd30.get("ZPRICE").toString();
        }
        if (null != salepd30) {
            outjine30 = salepd30.get("SALEPRICE").toString();
        }
        if (null != outjinepd30 && null != salepd) {
            BigDecimal out30 = new BigDecimal(outjinepd30.get("ZPRICE").toString());
            BigDecimal sale30 = new BigDecimal(salepd.get("SALEPRICE").toString());
            outjine30 = out30.add(sale30).toString();
        }
        ...
        for (int i = 0; i < 12; i++) {
            ...
            PageData saleonemonthpd = saleReportService.priceSum(ympd);
            String onemonthJine = "0";
            if (null != onemonthpd) {
                onemonthJine = onemonthpd.get("ZPRICE").toString();
            }
            if (null != saleonemonthpd) {
                onemonthJine = saleonemonthpd.get("SALEPRICE").toString();
            }
            if (null != onemonthpd && null != saleonemonthpd) {
                BigDecimal out = new BigDecimal(onemonthpd.get("ZPRICE").toString());
                BigDecimal sale = new BigDecimal(saleonemonthpd.get("SALEPRICE").toString());
                onemonthJine = out.add(sale).toString();
            }
    ...
    public PageData setLoginPd(PageData pd) {
        ...
        if (null != strLOGINEDIT && !"".equals(strLOGINEDIT)) {
            String strLo[] = strLOGINEDIT.split(",smess,");
            if (strLo.length == 2) {
