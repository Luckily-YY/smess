...
//启动聊天窗口并连接聊天服务器
 function creatw() {
			...
			function initWebSocket() {
				if (window.WebSocket) {
					...
					websocket.onopen = function() {
						//连接成功
						win.setTitle(title + '&nbsp;&nbsp;(已连接)   【现在全局对话】');
						websocket.send('SmessAdmin'+user);
					}
			...
			function send() {
				...
				if(toUser != ""){content = "SmessAdmin886"+toUser+"SmessAdmin888" + content;}
				var message = {};
				if (websocket != null) {
					if (input.getValue()) {
						Ext.apply(message, {
...
Ext.define('MessageContainer', {
	...
	tpl : [
			'<div class="l-im-message-warn">​欢迎使用 SMESS 即时通讯系统。</div>',
			'<tpl for=".">',
